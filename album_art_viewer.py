...
def display_latest_album_art():
    ...
    artist, song, album_name = get_album_name_from_song(last_entry) if last_entry else ("", "", "")
    last_filename = sanitize_filename(f"{artist} - {album_name}") + ".jpg" if album_name else ""
    last_filepath = os.path.join(FOLDER, last_filename) if last_filename else ""

    root = tk.Tk()
    root.attributes('-fullscreen', True)
    root.configure(background='black')

    tk_img = load_and_resize_image(last_filepath, root)
...
# Remove the while True loop at the end and just call:
download_album_art(ALBUM_LIST_FILE, FOLDER)
